...
import {
  ...
  PutAttenededLeadLost,
  UpdateAttendedPipelineStatus,
...
import InterviewStatusLeadFilter from "./interviewStatusFilter";

function InterviewStatusLeadPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.interviewStatusLead
  );
  ...
  const [joinedTentativestatus, setJoinedTentativeStatus] = useState(false);
  ...
  function submitCallRemainder(e) {
    ...
    setShowLoader(true);
    UpdateAttendedPipelineStatus(obj)
      .then((res) => {
        setShowLoader(false);
  ...
  const followDate1 = selectedDate.followUpDate1;
  const submitOfferaccepeted = (e) => {
    e.preventDefault();
    if (!selectedDate.tentativeJoinedDate) {
    ...
    let obj = {
      ...
      dates: formattedDateTime,
      id: leadDetails.id,
      tentativeJoinedDate: dateFormate(selectedDate.tentativeJoinedDate),
      // followDate1!= null?"followupdate1="+dateFormate(selectedDate.followUpDate1):"",
    };
    if (selectedDate.followUpDate1 != null) {
      obj.followup1date = dateFormate(followDate1);
    }
    setShowLoader(true);
    UpdateAttendedPipelineStatus(obj)
      .then((res) => {
    ...
    // PutCandidateJoinedFollowUp(leadDetails.id, selectedDate).then((date) => {
    //   if (date.code == 200) {
    //           } else {
  ...
  function submitconfirmed() {
    ...
    let obj = {
      // awaiting: true,
      awaiting: selectedOption,
      dates: formattedDateTime,
      // mn: leadDetails.mobileNumber,
      id: leadDetails.id,
    };
    UpdateAttendedPipelineStatus(obj).then(() => {
  ...
  function submitLeadLost() {
    PutAttenededLeadLost(leadDetails.mobileNumber).then(() => {
  ...
  const dateDiff = Math.ceil(
    (selectedDate.tentativeJoinedDate - new Date()) / (1000 * 60 * 60 * 24)
  );
            ...
            <div className={`${style.bodyCard} p-1 `}>
              <div className={`${style.textContent} p-1 `}>
                <div className="d-flex justify-content-between">
                  ...
                  {/* <div>
                    {dateFormate(el.interviewAttendingOn) ===
              ...
              childComponent={
                <>
                  <div>
                    ...
                    <div>
                      Are you sure that you want to awaiting{" "}
                      <b>
                        {" "}
                ...
                <>
                  <div style={{ width: "380px" }}>
                    ...
                    <form onSubmit={(e) => submitNotIntersted(e)}>
                      <div className="mt-2">
                        <span>
                          Are you sure that you want to not selected{" "}
                          <b>
                            {leadDetails.firstName} {leadDetails.lastName}
                          </b>{" "}
                          ?
                    ...
                    <form action="#" onSubmit={(e) => handleSubmitUpdate(e)}>
                      <div className="row mt-2">
                        <div className="col-sm-12 d-flex gap-3 align-item-center">
                          ...
                          <div>
                            <DatePicker
                              selected={selectedDate.tentativeJoinedDate}
                              onChange={(date) =>
                                handleDateChange(date, "tentativeJoinedDate")
                              }
                              minDate={new Date()}
                          ...
                          <div className="col-sm-12 mt-2 d-flex gap-3 align-item-center">
                            ...
                            <div>
                              <DatePicker
                                ...
                                maxDate={
                                  new Date(
                                    selectedDate.tentativeJoinedDate.getTime() -
                                      2 * 24 * 60 * 60 * 1000
                                  )
                                }
                                dateFormat="dd/MM/yyyy"
            ...
            childComponent={
              <>
                <div style={{ width: "400px" }}>
                  {" "}
                  <div className="d-flex justify-content-between align-items-center">
                    <h5 className="text-center">Joining Confirmation Date</h5>
                  ...
                  <form action="#" onSubmit={(e) => submitOfferaccepeted(e)}>
                    <div className="row mt-2">
                      <div className="col-sm-12 d-flex gap-3 align-item-center">
                        <label
                          htmlFor="currentpositionDropdown"
                        ...
                        <div>
                          <DatePicker
                            selected={selectedDate.tentativeJoinedDate}
                            onChange={(date) => {
                              console.log(date, "tetntativejoining");
                              handleDateChange(date, "tentativeJoinedDate");
                            }}
                            minDate={new Date()}
                            dateFormat="dd/MM/yyyy"
                        ...
                        <div className="col-sm-12 mt-2 d-flex gap-3 align-item-center">
                          ...
                          <div>
                            <DatePicker
                              selected={selectedDate.followUpDate1}
                              onChange={(date) => {
                                console.log(date, "follow up1 joining");
                                handleDateChange(date, "followUpDate1");
                              }}
                              minDate={new Date()}
                              maxDate={
...
export default InterviewStatusLeadPipeline;
