...
function ConnectingQualifyingFilter({ onclose }) {
  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.followUp1ConnectingQualifying
  );
  ...
  useEffect(() => {
    Dispatch(
      CandidatePipelineActions.setFollowUp1ConnectingQualifying(filterData)
    );
  }, [filterData]);
  ...
  function handleSubmit() {
    ...
    onclose();
    // console.log(CandidatePipelineActions.setFollowUp1ConnectingQualifying(filterData));
  }
...
export default ConnectingQualifyingFilter;
