...
import InterviewFollowUp1Filter from "./interviewFollowUp1Filter";
import { BsThreeDotsVertical } from "react-icons/bs";
import { LoadingButton } from "@mui/lab";
import { IoMdClose } from "react-icons/io";

function InterviewFollowUp1Pipeline() {
    ...
    <div className={`${style.pipelineContainer} mx-2`}>
      ...
      <div className={`${style.bodyContainer}`}>
        {leadData.data.map((el) => (
          ...
          >
            <div
              className={`${style.bodyCard} p-1 `}
              style={{
                border:
                  el.followUpDate1 === dateFormate(new Date())
                    ? "2.1px solid #169c50"
                    : "",
              }}
            >
                        ...
                        <span>
                          {(() => {
                            const differenceDays = Math.floor(
                              (new Date().getTime() -
                                new Date(el.followUpDate1).getTime()) /
                                (1000 * 60 * 60 * 24)
                            );
...
export default InterviewFollowUp1Pipeline;
