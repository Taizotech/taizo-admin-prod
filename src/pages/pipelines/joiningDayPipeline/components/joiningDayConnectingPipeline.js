...
import {
  ...
  UpdatejoiningDayPipelineStatus,
...
import JoiningDayConnectingFilter from "./joiningDayConnectingFilter";

function JoiningDayConnectingPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.joiningDayConnectingQualifying
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "joiningDayConnectingQualifying",
        value: page - 1,
      })
    );
  }
  ...
  function submitCallRemainder(e) {
    ...
    setShowLoader(true);
    UpdatejoiningDayPipelineStatus(obj)
      .then((res) => {
        setShowLoader(false);
  ...
  function submitconfirmed() {
    ...
    UpdatejoiningDayPipelineStatus(obj).then(() => {
  ...
  return (
    <div className={`${style.pipelineContainer} mx-2`}>
      <div className={`${style.headerCard} p-2 d-flex justify-content-between`}>
        <b> Connecting - Qualifying ({totalCount})</b>{" "}
        <div className="mt-1">
              ...
              childComponent={
                <>
                  <div>
                    ...
                    <div>
                      Are you sure that you want to joined this candidate{" "}
                      <b>
                        {" "}
...
export default JoiningDayConnectingPipeline;
