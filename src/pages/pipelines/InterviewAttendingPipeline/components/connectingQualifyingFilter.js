...
function AttendingConnectingQualifyingFilter({ onclose }) {
  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.attendingConnectingQualifying
  );
  ...
  useEffect(() => {
    Dispatch(
      CandidatePipelineActions.setCommonPipelineFilter({
        type: "attendingConnectingQualifying",
        data: filterData,
      })
    );
  }, [filterData]);
...
export default AttendingConnectingQualifyingFilter;
