...
function AttendingPipeline() {
  ...
  function submitLeadLost() {
    setenableSubmit(true);
    PutinterviewLeadLost(leadDetails.mobileNumber).then(() => {
  ...
  const handleSubmitUpdate = (e) => {
    ...
    if (!selectedDate.tentativeInterviewDate) {
      ...
      return;
    } else if (dateDiff >= 2 && !selectedDate.followUpDate1) {
      setValidity((prevState) => ({
        ...prevState,
        isFollowDate2: false,
      }));
...
export default AttendingPipeline;
